...
impl Editor {
    ...
    /// Public snapshot for compound edits driven from the UI layer
    /// (e.g. bundling via a file dialog).
    pub fn snapshot_public(&mut self) {
        self.snapshot();
    }
    ...
    pub fn mend(&mut self, project_dir: &Path, playhead: Time) -> bool {
        let index = match self.selected_clip() {
            Some((0, i)) => Some(i),
            Some(_) => None,
            None => ops::source_at(&self.project, playhead).map(|(i, _)| i),
        };
        let Some(index) = index else {
            self.message = "select a main-track clip (the cut before it mends)".into();
            return false;
        };
        self.snapshot();
        match viode_core::mend::mend_at(&mut self.project, project_dir, index, Time(250_000_000)) {
            Ok(i) => {
                self.dirty = true;
                self.selection = Selection::None;
                self.message = format!("mended the cut with a morph (clip {i})");
                true
            }
            Err(e) => {
                self.rollback(e);
    ...
    /// Chroma key on the selected overlay clip (None clears).
    pub fn set_matte(&mut self, method: Option<String>) -> bool {
        match self.selected_clip() {
            Some((t, _)) if t != 0 => {}
            _ => {
                self.message = "matte applies to overlay clips".into();
                return false;
            }
        }
        self.stage();
        self.clip_mut().unwrap().matte = method;
        true
    }

    /// Region mask on the selected clip (None clears).
    pub fn set_mask(&mut self, mask: Option<viode_core::Mask>) -> bool {
        if self.selected_clip().is_none() {
            return false;
        }
        if let Some(m) = &mask {
            if let Err(e) = viode_core::mask::validate(m) {
                self.message = e.to_string();
                return false;
            }
        }
        self.stage();
        self.clip_mut().unwrap().mask = mask;
        true
    }

    /// Match the selected clip's look to the clip before it.
    pub fn match_previous(&mut self, project_dir: &Path) -> bool {
        let Some((track, index)) = self.selected_clip() else {
            self.message = "select a clip to match".into();
            return false;
        };
        if index == 0 {
            self.message = "no previous clip to match against".into();
            return false;
        }
        let (tgt, rf) = {
            let clips = &self.project.tracks[track].clips;
            (clips[index].clone(), clips[index - 1].clone())
        };
        let abs = |src: &std::path::Path| {
            if src.is_absolute() { src.to_path_buf() } else { project_dir.join(src) }
        };
        let mid = |c: &viode_core::Clip| (c.in_.0 as f64 + c.out.0 as f64) / 2.0 / 1e9;
        let stats = viode_core::match_grade::frame_stats(&abs(&tgt.src), mid(&tgt))
            .and_then(|ts| {
                viode_core::match_grade::frame_stats(&abs(&rf.src), mid(&rf)).map(|rs| (ts, rs))
            });
        match stats {
            Ok((ts, rs)) => {
                ...
                self.dirty = true;
                self.message =
                    format!("matched: brightness {brightness:+.2}, saturation x{saturation:.2}");
                true
            }
            Err(e) => {
                self.message = e.to_string();
    ...
    pub fn key_add(&mut self, prop: &str, at: Time, value: f64) -> bool {
        if !["volume", "alpha", "x", "y", "scale"].contains(&prop) || value < 0.0 {
            self.message = "keyframes: volume, alpha, x, y, or scale; value >= 0".into();
            return false;
        }
        if self.selected_clip().is_none() {
            return false;

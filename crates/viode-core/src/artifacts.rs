//! Background thumbnail/waveform generation shared by the visual clients
//! (TUI and GUI). A worker thread runs ffmpeg so no UI ever blocks; results
//! are PNGs cached under cache/tui/, keyed by (source, in, out, pixel size)
//! so trims regenerate and reopens reuse. The directory name predates the
//! GUI and is kept so existing artifacts stay valid.
...
fn generate(job: &Job) -> Result<(), ()> {
    ...
    match s.kind {
        ...
        Kind::Wave => {
            crate::waveform_png(&s.src, in_, out, &job.dest, s.px_w.max(16), s.px_h.max(16), "0x7d92a8")
                .map_err(|_| ())
        }
    }
}
...
mod tests {
    ...
    fn keys_separate_kinds_ranges_and_sizes() {
        ...
        // A resize (new pixel budget) is a new artifact.
        assert_ne!(
            key_of(&spec(Kind::Strip, 0.0, 1.0, 480)),

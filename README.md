...
Prebuilt binaries for Linux (x86_64) and Apple-silicon Macs are
attached to [every release](https://github.com/edimoldovan/viode/releases/latest):
unpack the archive and put `viode` on your PATH. The binaries need the
engine installed alongside (the requirements below) — run
`viode doctor` afterwards and it will name anything missing.
